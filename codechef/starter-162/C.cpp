...
void testcase() {
  ...
  string s;
  cin >> n >> s;
  vector<pair<char, int>> rle = run_length_encoding<char>(s);
  ll ans = 0;

  while ((ll)rle.size() > 0) {
    ll id = 0;

    for (ll i = 0; i < (ll)rle.size(); i++)
      if (rle[id].second > rle[i].second)
        id = i;

    for (ll i = 0; i < (ll)rle.size(); i++)
      if (id != i)
        rle[i].second -= rle[id].second;

    ans += rle[id].second;
    rle.erase(rle.begin() + id);

    if (id != 0 && id != (ll)rle.size() - 1) {
      rle[id - 1].second += rle[id].second;
      rle.erase(rle.begin() + id);
    }
  }
